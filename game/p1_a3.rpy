label p1_a3:
    play music "audio/feliz.mp3" volume 0.2
    scene bg_casa_papa
    show car_n_bestido
    show car_n_bestido:
        subpixel True pos (0.2, 1.1) xzoom 0.9 yzoom 0.9 
    with dissolve
    voice "voz26.mp3"
    carp "(A pesar de mi pesadilla, de niña vivía muy bien porque yo era la mirada misma de mi papá.)"
    voice "voz27.mp3"
    carp "(Él me hablaba con los ojos y yo ya entendía lo que él no decía con la boca. Éramos muy unidos. A veces sentía que éramos la misma persona.)"
    voice "voz28.mp3"
    carp "(Era un hombre bueno en su casa y con los vecinos, a pesar de que la Violencia lo había marcado muy adentro.)"

    show papa_formal
    
    show papa_formal:
        subpixel True pos (0.7, 2.0) zpos 0.0 xzoom 2.0 yzoom 2.0
    voice "voz29.mp3"
    papa "Ojalá a nosotros nunca nos toque ni siquiera una uña, por eso yo le pido a Dios, cada domingo, que no nos vaya a coger la guerra,"
    voice "voz30.mp3"
    papa "como antes que me toco esconderme en el monte para evitar que me asesinaran los chulavitas."
    voice "voz31.mp3"
    papa "Yo estuve enmontado como un animal, pasando a la guerrilla"
    voice "voz32.mp3"
    papa "y recorrí todo eso del Cañón de las Hermosas, del río Amoyá para arriba a salir por el páramo hasta el río Atá, y por el río Amoyá para abajo hasta salir al Saldaña. "   
    voice "voz33.mp3"
    papa "Entregué mis armas al ejército a cambio de nada, cuando Rojas medio cuadro  eso, esa vez no consegui nada, pero en esas movidas me encontré con su mamá. "

    hide papa_formal

    show mama_formal
    
    show mama_formal:
        subpixel True pos (0.7, 2.0) zpos 0.0 xzoom 2.0 yzoom 2.0 
    voice "voz34.mp3"
    mama "El aun era del monte cuando lo conoci, pasaba a veces y me conversaba, me preguntaba por la gente y se volvia a ir. "
    voice "voz35.mp3"
    mama "Siempre se me presentaba sin armas, con su ropa de civil, y ella, cuando él pasaba, pensaba que no volvería a verlo. "
    voice "voz36.mp3"
    mama "Hasta que una vez, cuando ya se había entregado, se le presentó en forma de retirado. Él decía, por coquetearle a mi mamá, que la culpa de esa entrega había sido de ella, por tener esos ojos como los tenía. "
return





